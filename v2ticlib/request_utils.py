...
def get_max_transcription_length(request:dict) -> int:
    default_value = metadata_utils.get_default_max_transcription_length()
...
def is_log_transcriptions_enabled(request:dict) -> bool:
    default_value = metadata_utils.is_log_transcriptions_enabled()
...
def respond_with_audio_enabled(request:dict) -> bool:
    default_value = metadata_utils.respond_with_audio_enabled()
    return get_field_value_or_default(request, Fields.RESPOND_WITH_AUDIO_ENABLED, default_value, literal_eval=True)
