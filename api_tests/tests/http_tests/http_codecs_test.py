...
from api_tests.core.file_utils import get_audio_text,read_test_data
...
@pytest.mark.httpcodecs
@pytest.mark.httpregression
def test_audio_transcription_for_codecs(http_codecs_test_data_tc):
    test_data_tc = http_codecs_test_data_tc
    ...
    with allure.step(f"POST transcribe request to server for language {audio_lang}"):
        ...
        validate("Python/3.9 aiohttp/3.8.5", acknowledgement_response.headers.get("server"), 'Validate Response Header server name')
    ...
    with allure.step(f"GET scrid {scrid} response from HTTP Listener"):
        ...
        validate("application/xml; charset=utf-8", final_response.headers.get("Content-Type"),
        ...
        response_body_element_key = ['scrid', 'externalRequestID', 'reasonStatus', 'recognitionStatus', 'globalConfidenceScore']
        ...
        validate_numeric(rtf_max,'>',final_response.headers.get('X-RTF'),'Validate Response Header X-RTF')
        validate_numeric(global_confidence_score_min,'<',xml_response_dict['globalConfidenceScore'], 'Validate Response Body globalConfidenceScore')