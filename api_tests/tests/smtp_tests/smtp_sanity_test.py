...
from api_tests.core.assert_utils import validate,validate_numeric
...
async def test_send_email(smtp_listener_fixture,smtp_sanity_test_data_tc):
    handler = smtp_listener_fixture

    # Test Data
    headers = get_smtp_default_headers()
    test_data_tc = smtp_sanity_test_data_tc
    ...
    global_confidence_score_min = test_data_tc['verify_global_confidence_score_min']
    rtf_max = test_data_tc['verify_rtf_max']


    # Send email to IC server
    actual_email_sent_message = await send_email_with_audio(sender_email, receiver_email, subject, headers,
                                                            audio_language, formatted_message)
    ...
    validate(test_data_tc['input_audio_language'],received_email.email_headers['requested_languages'].strip(),'Validate requested language')
    validate(test_data_tc['verify_response_conversion_status'],received_email.email_headers['conversion_status'].strip(),'Validate conversion status')
    validate_numeric(rtf_max,'>',received_email.email_headers['rtf'].strip(),'Validate RTF')
    validate_numeric(global_confidence_score_min,'<',received_email.email_headers['global_confidence_score'].strip(), 'Validate globalConfidenceScore')