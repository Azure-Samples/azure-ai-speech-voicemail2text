...
class SmtpEjector(Ejector):
    ...
    async def eject(self,request:dict, response:dict) -> str:
        ...
        try:
            return await self.do_eject(host, port, mail_from, rcpt_to, headers, content)
        except Exception as e:
            log(self.do_eject.retry.statistics)
            raise e

    @retry_with_custom_strategy(Constants.EJECTOR)
    async def do_eject(self, host, port, mail_from, rcpt_to, headers, content):
        timeout = self.get_timeout()
        return await smtp_util.send(host, port, mail_from, rcpt_to, headers, content, timeout)
